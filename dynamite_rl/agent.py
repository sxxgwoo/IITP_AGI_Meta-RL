...
#             # --------------------------------------
#             # B. KL with latent belief conditioning (session-aware)
#             #    - 같은 세션: prior = q_{t-1}
#             #    - 새로운 세션 시작: prior = N(0, I)
#             # --------------------------------------

#             B, T, Dz = mu.shape
#             device = mu.device

#             # 이전 step posterior (candidate prior)
#             mu_prev = torch.zeros_like(mu)
#             logvar_prev = torch.zeros_like(logvar)
#             mu_prev[:, 1:, :] = mu[:, :-1, :].detach()
#             logvar_prev[:, 1:, :] = logvar[:, :-1, :].detach()

#             # same_session[t] = (t > 0 이고, t와 t-1이 같은 session인지 여부)
#             # session_id는 위에서
#             #   term = terminations.squeeze(-1) * masks.squeeze(-1)
#             #   session_id = torch.cumsum(term, dim=1).long()
#             # 로 이미 계산되어 있음
#             same_session = torch.zeros(B, T, dtype=torch.bool, device=device)
#             same_session[:, 1:] = (session_id[:, 1:] == session_id[:, :-1])

#             # prior 기본값은 N(0, I)  (mu=0, logvar=0 ⇒ var=1)
#             prior_mu = torch.zeros_like(mu)
#             prior_logvar = torch.zeros_like(logvar)

#             # 같은 세션인 위치에만 이전 posterior를 prior로 덮어쓰기
#             prior_mu[same_session] = mu_prev[same_session]
#             prior_logvar[same_session] = logvar_prev[same_session]
...
class DynaMITEAgent:
    def __init__(self, vae, actor_critic, config):
        ...
        self.opt_vae = torch.optim.Adam(self.vae.parameters(), lr=config.lr_vae)
        self.opt_rl = torch.optim.Adam(self.actor_critic.parameters(), lr=config.lr_policy)
    ...
    def update(self, rollouts):
        ...
        # ---------------------------------------------
        # term / session_id는 모든 VAE epoch에서 공통으로 쓰이므로 미리 계산
        # term[t] = "t step에서 session_changed가 발생했는가"
        # session_id[t] = sum_{k < t} term[k]
        #   → term_shift로 한 칸 밀어서 "t+1부터 새 세션 시작" 맞추기
        # ---------------------------------------------
        term = terminations.squeeze(-1) * masks.squeeze(-1)  # (B, T)

        term_shift = torch.zeros_like(term)
        term_shift[:, 1:] = term[:, :-1]                     # (B, T)
        session_id = torch.cumsum(term_shift, dim=1).long()  # (B, T)
        ...
        for _ in range(self.cfg.vae_epochs):
            # encode: (B,T,·) → (B,T,Dz)
            mu, logvar, term_logits, _ = self.vae.encode(states, prev_actions, rewards)
            z = self.vae.reparameterize(mu, logvar)          # (B, T, Dz)
            ...
            pair_mask = (same_session & pair_valid)                        # (B, T, T)
            pair_mask_f = pair_mask.to(states.dtype)                       # float mask

            # pair (i,j) 에 대해 "z_i로 reward_j 재구성"
            z_pair = z.unsqueeze(2).expand(B, T, T, z.size(-1))
            states_pair = states.unsqueeze(1).expand(B, T, T, states.size(-1))
            actions_pair = actions.unsqueeze(1).expand(B, T, T, actions.size(-1))
            rewards_pair = rewards.unsqueeze(1).expand(B, T, T, 1)
            ...
            # --------------------------------------
            # B. KL with latent belief conditioning (session-aware)
            #    - 같은 session: prior = q_{t-1}
            #    - 새 session 시작: prior = N(0, I)
            # --------------------------------------
            B_, T_, Dz = mu.shape
            ...
            same_sess = torch.zeros(B_, T_, dtype=torch.bool, device=device)
            same_sess[:, 1:] = (session_id[:, 1:] == session_id[:, :-1])

            prior_mu = torch.zeros_like(mu)       # N(0, I) 기본
            prior_logvar = torch.zeros_like(logvar)

            prior_mu[:, 1:, :] = torch.where(
                same_sess[:, 1:].unsqueeze(-1),
                mu_shift[:, 1:, :],
                torch.zeros_like(mu_shift[:, 1:, :])
            )
            prior_logvar[:, 1:, :] = torch.where(
                same_sess[:, 1:].unsqueeze(-1),
                logvar_shift[:, 1:, :],
                torch.zeros_like(logvar_shift[:, 1:, :])
            )
            ...
            # C. Consistency Loss (논문식)
            #    θ_t = KL(q_t || q_end(session))
            #    L_cons = max(θ_{t+1} - θ_t, 0) within same session
            # --------------------------------------
            session_last_idx = torch.zeros_like(session_id)  # (B, T)
            session_last_idx[:, -1] = T - 1
            ...
            batch_indices = torch.arange(B, device=device).unsqueeze(1).expand(B, T)
            mu_end = mu[batch_indices, session_last_idx]          # (B, T, Dz)
            logvar_end = logvar[batch_indices, session_last_idx]  # (B, T, Dz)
            ...
            last_vae_loss_value = loss_vae.item()

        # ---------------------------------------------------
        # 2. Train Policy (PPO) - 최신 VAE로 z 다시 계산
        # ---------------------------------------------------
        with torch.no_grad():
            mu, logvar, _, _ = self.vae.encode(states, prev_actions, rewards)
            z = self.vae.reparameterize(mu, logvar)
